...
func (r *SearchReconciler) reconcileGlobalSearch(ctx context.Context,
	...
	if instance.ObjectMeta.Annotations["global-search-preview"] == "true" {
		...
		log.V(3).Info("The global-search-preview annotation is not present. Checking if global search was enabled before.")
...
func (r *SearchReconciler) enableGlobalSearch(ctx context.Context, instance *searchv1alpha1.Search) error {
	...
	if err == nil && clusterList != nil {
		for _, cluster := range clusterList.Items {
			...
			err = r.createManifestWork(ctx, cluster.GetName(), instance.GetNamespace())
...
func searchAPIEnvVar(instance *searchv1alpha1.Search, inputEnv corev1.EnvVar) bool {
	...
	if enabled {
		...
		} else if existingEnv.Value != inputEnv.Value {
			// Env exists, but needs an update to value.
			instance.Spec.Deployments.QueryAPI.Env[existingIndex].Value = inputEnv.Value
			return false
		} else {
